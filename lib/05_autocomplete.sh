...
autocomplete_init() {
  ...
  declare -gA DT_AUTOCOMPLETIONS=() && \
  declare -ga DT_AUTOCOMPLETE=() && \
  declare -ga DT_AUTOGENERATED=()
}

autocomplete_add_key() {
  local key=$1
  eval "DT_AUTOCOMPLETIONS[${key}]="
}

autocomplete_reg_family() {
  DT_AUTOCOMPLETE+=($1)
  local cmd_family=$1
  eval "DT_AUTOCOMPLETIONS[${cmd_family}]="
}
