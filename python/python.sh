...
function python_build() {
  export
  dt_exec_or_echo "make -f ${PYMAKE} python3"
}

function python_venv_init() {
    dt_exec_or_echo "make -f ${PYMAKE} venv-init"
}

function python_pip_init() {
  export SITE_PACKAGES="$("${VPYTHON}" -m pip show pip | grep Location | cut -d':' -f 2)"
  dt_exec_or_echo "make -f ${PYMAKE} pip-init"
}

function python_venv_clean() {
  dt_exec_or_echo "make -f ${PYMAKE} venv-clean"
}

function python_pip_clean() {
  export SITE_PACKAGES="$("${VPYTHON}" -m pip show pip | grep Location | cut -d':' -f 2)"
  dt_exec_or_echo "make -f ${PYMAKE} pip-clean"
}
