function psql_conn() {
  ...
  dt_err_if_empty ${fname} "PG_DIR"; err=$?; if [ "${err}" != 0 ]; then return ${err}; fi
...
function _psql_init() {
  (
    ...
    migrator=$2; dt_err_if_empty ${fname} "migrator"; err=$?; if [ "${err}" != 0 ]; then return ${err}; fi
    app=$3; dt_err_if_empty ${fname} "app"; err=$?; if [ "${err}" != 0 ]; then return ${err}; fi
