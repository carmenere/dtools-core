...
function rustup_install() {
  ...
  dt_err_if_empty "RUSTUP_TOOLCHAIN" "${RUSTUP_TOOLCHAIN}"; exit_on_err ${fname} $? || return $?
  dt_err_if_empty "RUSTUP_TARGET_TRIPLE" "${RUSTUP_TARGET_TRIPLE}"; exit_on_err ${fname} $? || return $?
...
function rustup_nightly_install() {
  local fname=$(dt_fname "${FUNCNAME[0]}" "$0")
  dt_err_if_empty "NIGHTLY_VERSION" "${NIGHTLY_VERSION}"; exit_on_err ${fname} $? || return $?
  dt_err_if_empty "RUSTUP_TARGET_TRIPLE" "${RUSTUP_TARGET_TRIPLE}"; exit_on_err ${fname} $? || return $?
...
function rustup_component_add() {
  local fname=$(dt_fname "${FUNCNAME[0]}" "$0")
  dt_err_if_empty "RUSTUP_COMPONENTS" "${RUSTUP_COMPONENTS}"; exit_on_err ${fname} $? || return $?
