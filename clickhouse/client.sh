...
function clickhouse_conn_admin() {( ctx_clickhouse_admin && clickhouse_conn; )}
function clickhouse_conn_app() {( ctx_clickhouse_app && clickhouse_conn; )}
function clickhouse_conn_admin() {( ctx_clickhouse_admin && clickhouse_conn; )}
function clickhouse_conn_app() {( ctx_clickhouse_app && clickhouse_conn; )}
...
function clickhouse_conn_docker_admin() {(
  ...
  ctx_clickhouse_admin && clickhouse_conn
)}

function clickhouse_conn_docker_app() {(
  docker_service_check_clickhouse && \
  ctx_clickhouse_app && clickhouse_conn
)}
